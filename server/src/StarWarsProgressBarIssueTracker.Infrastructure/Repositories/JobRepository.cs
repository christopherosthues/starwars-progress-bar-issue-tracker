...
{
    public override async Task<List<DbJob>> GetAllAsync(CancellationToken cancellationToken = default)
    {
        return await DbSet.OrderBy(job => job.JobType).ToListAsync(cancellationToken);
    }
