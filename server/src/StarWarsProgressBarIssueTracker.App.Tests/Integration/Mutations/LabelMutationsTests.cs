...
public class LabelMutationsTests : IntegrationTestBase
{
    ...
    public async Task AddLabelShouldAddLabel(Label expectedLabel)
    {
        ...
        // Act
        GraphQLResponse<AddLabelResponse> response = await graphQlHttpClient.SendMutationAsync<AddLabelResponse>(mutationRequest);
    ...
    public async Task UpdateLabelShouldUpdateLabel(Label expectedLabel)
    {
        ...
        // Act
        GraphQLResponse<UpdateLabelResponse> response = await graphQlHttpClient.SendMutationAsync<UpdateLabelResponse>(mutationRequest);
    ...
    public async Task DeleteLabelShouldDeleteLabel()
    {
        ...
        // Act
        GraphQLResponse<DeleteLabelResponse> response = await graphQlHttpClient.SendMutationAsync<DeleteLabelResponse>(mutationRequest);
    ...
    public static IEnumerable<Func<(Label, IEnumerable<string>)>> InvalidAddLabelCases()
    {
        ...
        yield return () => (new Label { Title = new string('a', 51), Description = null, Color = "#001122", TextColor = "#334455" }, new List<string> { $"The value '{new string('a', 51)}' for {nameof(Label.Title)} is long short. The length of {nameof(Label.Title)} has to be between 1 and 50." });
        yield return () => (new Label { Title = "Valid", Description = new string('a', 256), Color = "#001122", TextColor = "#334455" }, new List<string> { $"The value '{new string('a', 256)}' for {nameof(Label.Description)} is long short. The length of {nameof(Label.Description)} has to be less than 256." });
