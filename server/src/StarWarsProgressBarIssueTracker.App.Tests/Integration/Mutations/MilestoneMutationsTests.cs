...
public class MilestoneMutationsTests : IntegrationTestBase
{
    ...
    public async Task AddMilestoneShouldAddMilestone(Milestone expectedMilestone)
    {
        ...
        // Act
        GraphQLResponse<AddMilestoneResponse> response = await graphQlHttpClient.SendMutationAsync<AddMilestoneResponse>(mutationRequest);
    ...
    public async Task UpdateMilestoneShouldUpdateMilestone(Milestone expectedMilestone)
    {
        ...
        // Act
        GraphQLResponse<UpdateMilestoneResponse> response = await graphQlHttpClient.SendMutationAsync<UpdateMilestoneResponse>(mutationRequest);
    ...
    public async Task DeleteMilestoneShouldDeleteMilestone()
    {
        ...
        // Act
        GraphQLResponse<DeleteMilestoneResponse> response = await graphQlHttpClient.SendMutationAsync<DeleteMilestoneResponse>(mutationRequest);
    ...
    public static IEnumerable<Func<(Milestone, IEnumerable<string>)>> InvalidAddMilestoneCases()
    {
        ...
        yield return () => (new Milestone { Title = new string('a', 51), Description = null, State = MilestoneState.Open }, new List<string> { $"The value '{new string('a', 51)}' for {nameof(Milestone.Title)} is long short. The length of {nameof(Milestone.Title)} has to be between 1 and 50." });
        yield return () => (new Milestone { Title = "Valid", Description = new string('a', 256), State = MilestoneState.Open }, new List<string> { $"The value '{new string('a', 256)}' for {nameof(Milestone.Description)} is long short. The length of {nameof(Milestone.Description)} has to be less than 256." });
    }
