...
public class AppearanceMutationsTests : IntegrationTestBase
{
    ...
    public static IEnumerable<Func<(Appearance, IEnumerable<string>)>> InvalidAddAppearanceCases()
    {
        ...
        yield return () => (new Appearance { Title = new string('a', 51), Description = null, Color = "#001122", TextColor = "#334455" }, new List<string> { $"The value '{new string('a', 51)}' for {nameof(Appearance.Title)} is long short. The length of {nameof(Appearance.Title)} has to be between 1 and 50." });
        yield return () => (new Appearance { Title = "Valid", Description = new string('a', 256), Color = "#001122", TextColor = "#334455" }, new List<string> { $"The value '{new string('a', 256)}' for {nameof(Appearance.Description)} is long short. The length of {nameof(Appearance.Description)} has to be less than 256." });
